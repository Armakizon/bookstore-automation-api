...
def test_typing_full_book_title_shows_correct_result(driver):
    # TC_111 - Typing full book title shows correct result
    ...
    half_title = "Git"
    search_box.send_keys(half_title)
    ...
    assert len(visible_books) >= 1, f"Expected at least 1 result, got {len(visible_books)}"
    assert any(full_title in book for book in visible_books), f"Book title '{full_title}' not found in any result"
